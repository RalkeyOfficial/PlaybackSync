...
| [`src/services/metadataApi.ts`](../src/services/metadataApi.ts)             | URL → metadata lookup (single-mode seed) | No   | **Yes**       |
...
[`src/components/RoomCreateDialog.vue`](../src/components/RoomCreateDialog.vue) is the form for creating a room. It uses `NcDialog` for the chrome, `NcTextField` for the name and bootstrap-URL inputs, and `NcSelect` for the TTL preset. The validation logic is intentionally lightweight on the client — it checks that the bootstrap URL looks roughly like an http(s) URL — because the backend service is the authoritative validator. We don't want two different sources of truth telling the user different things about what's valid.

The dialog also owns the **playlist-behaviour picker** and the **single-mode seed flow**. The picker is a three-option radio group (`NcCheckboxRadioSwitch type="radio"` sharing one `name`) — Default / Single / Freeform, defaulting to Default — each with an explanatory hint line. A single `mode` ref (`'default' | 'single' | 'freeform'`) is the source of truth; the `singleMode` / `freeformMode` booleans the create API expects are `computed` from it, and mutual exclusion is inherent to the radio group rather than enforced by disabling switches. Above the picker, a **mode suggestion** `NcNoteCard` nudges toward Freeform when the bootstrap URL looks like a standalone video (classified by [`src/util/suggestMode.ts`](../src/util/suggestMode.ts): a bare YouTube/Vimeo watch URL → freeform; a YouTube `?list=`, miruro, or Crunchyroll URL → default/no nudge); it offers a one-click "Use freeform mode" plus "Dismiss", never auto-overrides the choice, and re-arms when the URL changes (see [`agent-os/specs/2026-05-30-1110-suggest-mode-from-bootstrap-url/`](../agent-os/specs/2026-05-30-1110-suggest-mode-from-bootstrap-url/)). When `singleMode` is on, the bootstrap-URL field's helper text changes — it now describes the one video everyone will watch — and a seed-entry block appears beneath it: a debounced 400ms call to `POST /api/v1/metadata/lookup` resolves the URL, an inline preview row shows the provider chip + fetched title (or "Title not found, will use URL" on oEmbed failure, or "Could not detect a video on this page" when the URL itself fails to parse), and an owner-editable `Video title` `NcTextField` is pre-filled with the fetched title (a `labelTouched` ref makes sure later async results don't stomp on what the owner typed). Submit attaches `initialEntries: [{ providerId, videoId, pageUrl, label, source: 'curated' }]`, aligning `bootstrapUrl` with that one entry's `pageUrl` — the contract a single-mode room is persisted with (see [`agent-os/specs/2026-05-16-1500-content-model-single-mode/`](../agent-os/specs/2026-05-16-1500-content-model-single-mode/)).
...
[`src/components/RoomDetailDialog.vue`](../src/components/RoomDetailDialog.vue) is the room detail and live-control modal. It opens from the room card and re-fetches via `roomsApi.getRoom(uuid)` so the live block (presence, playback state) and the persisted cursor reflect current state rather than whatever was cached in the rooms list. Layout is split into tabs — Overview / Playlist / Event log — with the overview tab carrying the status strip, bootstrap URL, share link, timestamps, **Playback**, **Connected viewers**, **Now watching** (the playlist entry referenced by `room.cursorEntryId`), and identifier.

The Playlist tab embeds [`src/components/PlaylistEditor.vue`](../src/components/PlaylistEditor.vue) — the dashboard surface for the playlist + cursor substrate. Its header carries a small colored chip (default = primary, single = warning, freeform = success) plus an `NcSelect` mode picker that calls `roomsStore.updateSettings()` on change; switching *to* single mode while the playlist has more than one entry is intercepted by a confirmation dialog ("Lock the playlist?") that mirrors the existing "Clear playlist?" pattern. Below the header is the entry list — per-row cursor dot, `S{season}E{episode}` (or `#{position}`) prefix, label, provider + source chips, stale dimming for entries `lastSeenAt` older than seven days, an open-in-tab link, and an `NcActions` menu (`Move cursor here`, `Edit label`, `Convert to curated`, `Move up`, `Move down`, `Remove`). "Add entry" and "Clear all" sit in the header; both disable with a "Locked by single mode." tooltip when single mode is on. Removing the cursor entry is pre-flighted client-side — the editor surfaces an inline `NcNoteCard` warning instead of firing a doomed DELETE.
