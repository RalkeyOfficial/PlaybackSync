...
[`src/components/RoomDetailDialog.vue`](../src/components/RoomDetailDialog.vue) is the room detail and live-control modal. It opens from the room card and re-fetches via `roomsApi.getRoom(uuid)` so the live block (presence, playback state, content identity) reflects the daemon's current state rather than whatever was cached in the rooms list. Layout is split into discrete sections: status strip, target URL, share link, timestamps, **Playback**, **Connected viewers**, **Now watching**, and identifier.
