...
Symptom: the browser extension scrapes an episode list, but the room's playlist stays empty — entries only appear after a full dashboard page reload, and a freshly-created room won't even seed its cursor from the joining tab. The extension *is* sending the data (you can confirm a populated `catalogFragment` in the `JOIN` frame via the extension's service-worker → Network → WS → Messages), so the loss is server-side: the daemon receives the entries but the playlist merge throws.

`JoinHandler` treats `catalogFragment` (and the empty-playlist cursor seed) as best-effort — it logs the failure and continues — so the only visible symptom is the empty playlist. `ROOM_STATE` still returns normally because the failed merge is the first DB write; the read-only state reply is unaffected. The failure is swallowed, so **read the logged reason** — it's logged at `warning`, so it shows at the default loglevel (each `nextcloud.log` line is one long JSON object, so extract just the message):

```bash
docker exec <nextcloud-container> sh -c "grep -o '\[playbacksync ws\][^\"]*' /var/www/html/data/nextcloud.log | grep -iE 'skipped' | tail"
```

Then match the reason after `merge skipped:` / `seed skipped:`:

- **`Call to undefined method OC\DB\ResultAdapter::fetchAssociative()`** — a Nextcloud-version incompatibility, **fixed in 1.0.4-beta1**. `RoomMapper::lockRoomForUpdate` used Doctrine's `fetchAssociative()`, which NC 34's result adapter exposes but NC 32's does not, so every playlist merge fatally errored on NC 32. The fix uses the portable `IResult::fetch()`. If you see this, you're on an older build — update. *This was the original empty-playlist-on-NC-32 bug.*
- **`SQLSTATE[HY000]: … MySQL server has gone away`** — the daemon's DB connection was reaped while idle (it only touches the DB on `JOIN`/playlist mutations, so it can sit idle past the server's `wait_timeout`), and a merge transaction can't transparently reconnect on a dead socket. The daemon pings the DB every 30 s to keep the connection warm (see [Operational notes](#operational-notes)), so this shouldn't recur on a current build. Stopgap: `docker restart playbacksync-ws` forces a fresh connection.
- **`single_mode_locked`, a cap exception, etc.** — not a bug; the merge was correctly refused, and the reason says why.

> **If a redeploy doesn't take effect**, suspect deployment, not code: the new files didn't land where the daemon loads the app from, or the daemon process wasn't restarted. Verify the load path with `occ app:getpath playbacksync` (is your deploy writing *there*?), and check the **file** the daemon loads rather than the log tail — old `skipped` lines linger in `nextcloud.log` indefinitely, so a stale line is not proof the bug is still live:
> ```bash
> docker exec playbacksync-ws sh -c 'p=$(php /var/www/html/occ app:getpath playbacksync); grep -n "fetchAssociative\|->fetch()" "$p/lib/Db/RoomMapper.php"'
> ```
...
- **The daemon keeps its DB connection warm.** `Tick` pings the database with a trivial `SELECT 1` every 30 s (`DB_KEEPALIVE_INTERVAL_MS`) and `close()`s the connection on failure so the next query reconnects lazily. The daemon only touches the DB on `JOIN`/playlist mutations, so without this it can sit idle past the database's `wait_timeout`, lose its connection, and have the next playlist-merge transaction (which can't transparently reconnect) fail with "MySQL server has gone away". This is **defensive hardening** against that idle-timeout case — it was *not* the cause of the original empty-playlist bug (that was the NC 32 `fetchAssociative` incompatibility; see the troubleshooting entry above).
