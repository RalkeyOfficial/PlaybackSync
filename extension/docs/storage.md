...
The credentials are also treated as **sticky**: when the navigation-guard pulls a tab back to the room cursor (see [architecture.md](architecture.md)), the hard-nav target is built with `sync_url` / `sync_password` re-attached from the stored creds into the fragment (`withCredentialParams` in [`entrypoints/background.ts`](../entrypoints/background.ts)). The cursor's `pageUrl` is the canonical credential-free form, so without this a reload after a dropped socket couldn't re-hand-off. This is enforced **only at the guard's hard-nav pull-back**, not on in-playlist episode clicks or server-driven cursor changes (those route through the site's own SPA navigation). Re-attaching the creds is harmless: the guard's URL matching ignores the fragment when deciding playlist membership, so a pull-back never loops.
