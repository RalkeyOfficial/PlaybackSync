...
The hard-nav target is built with the share-link credentials re-attached into its fragment: `withCredentialParams` sets `sync_url` / `sync_password` from the stored creds onto the cursor's (canonical, credential-free) `pageUrl`. The creds go in the fragment for the same reason the share redirect uses one — sites like miruro strip unknown *query* params on a server-side redirect, but never see the fragment — so a reload after a dropped socket can still re-hand-off (see [`storage.md`](storage.md)). This stickiness is enforced only here, at the guard's hard-nav; in-playlist clicks and server-driven cursor changes route through the site's own SPA navigation.
