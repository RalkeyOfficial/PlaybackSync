...
- **Periodic cursor reconciliation (URL-identity sites).** A per-guarded-tab ~5s timer (`RECONCILE_INTERVAL_MS`, `reconcileTimers`, armed alongside `navGuardedTabs`) runs `reconcileCursorToRoom`: it resolves the tab's *live* URL to a `videoId` and, if it has drifted off the room cursor, pulls the tab back — lightweight `pullTabBackToCursor` for an in-playlist episode, hard-reload `recheckAndPullBack` off-playlist/cross-site. It is a generic backstop — cause-agnostic — and only ever **converges toward the cursor**; it never forwards a new cursor (that stays tied to real user navigation). It shares the guard's convergence/settle gate, so it no-ops on a healthy tab and gives a legitimate in-flight episode change its round-trip (the ~3s `forwardCursorChangeFromNav` fallback window) to land before it would undo it. Freeform tabs are not reconciled (non-coercive).
