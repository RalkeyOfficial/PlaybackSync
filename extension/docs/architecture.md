...
Two mechanisms enforce the anchor; **which one an adapter uses depends on whether its URLs encode video identity**, and they must not both run for the same site (they'd double-fire):

- **The navigation-guard (URL-identity sites, opt-in).** A `chrome.tabs.onUpdated` listener — browser-level, so it sees main-world SPA URL changes the isolated-world content script can't. It resolves the tab's new URL to a `videoId` and routes per mode (`routeGuardedNav`, mirroring `handleCursorTrigger`): on-cursor → no-op (loop-stop); freeform-resolvable & default-in-playlist → `CURSOR_CHANGE_REQUEST` (`forwardCursorChangeFromNav`, which un-converges the tab so the new player's autoplay/seek is dropped until the room re-converges); single-in-playlist → lightweight `pullTabBackToCursor`; off-playlist/cross-site → hard-reload `recheckAndPullBack`. Because every episode change also changes the URL, this single detector covers *every* control — episode list, "Next episode" button, keyboard, autoplay-advance — plus every departure (home link, thumbnails, address bar, back/forward, redirect, cross-site). Opt-in per adapter via `Adapter.guardNavigation` (the flag rides the `identity` message; the background stores `navGuardedTabs`). Miruro uses this and ships no DOM click listener.
- **Cursor-trigger arbitration (static-URL sites).** For sites whose URL *doesn't* change (or doesn't encode identity) between videos, the guard can't help. There the adapter's in-page DOM click listener calls `emitCursorTrigger`, and the background decides per mode + playlist (`handleCursorTrigger`): default-in-playlist & freeform → `CURSOR_CHANGE_REQUEST`; default-off-list & single-mismatch → `pullTabBackToCursor` (synth-click, `location.href` fallback). This is the only signal such sites have; they omit `guardNavigation`.

Both are **event-driven**, so neither recovers a tab that drifts and then sits still — a lost/rejected `CURSOR_CHANGE_REQUEST`, a missed `CURSOR_CHANGE` broadcast, a failed synth-click. (The ~5s heartbeat can't help: `HEARTBEAT`/`SYNC_ADJUST` carry only playback position, never the cursor, so a wrong-episode tab just gets its wrong video seeked to the room's position — masking the split.) A third mechanism closes that gap:

- **Periodic cursor reconciliation (URL-identity sites).** A per-guarded-tab ~5s timer (`RECONCILE_INTERVAL_MS`, `reconcileTimers`, armed alongside `navGuardedTabs`) runs `reconcileCursorToRoom`: it resolves the tab's *live* URL to a `videoId` and, if it has drifted off the room cursor, pulls the tab back — lightweight `pullTabBackToCursor` for an in-playlist episode, hard-reload `recheckAndPullBack` off-playlist/cross-site. It is a generic backstop — cause-agnostic — and only ever **converges toward the cursor**; it never forwards a new cursor (that stays tied to real user navigation). It stays dormant while a legitimate episode change is in flight — both the transient convergence/settle gate and, crucially, a `pendingCursorTarget !== null` check — so a real forward gets its round-trip (the ~3s `forwardCursorChangeFromNav` fallback window) to land before the reconciler could undo it. Without the pending check it would yank the tab back to the room cursor the moment an earlier change's broadcast re-converged the tab, while the user's newer change was still round-tripping. Freeform tabs are not reconciled (non-coercive).

Both event mechanisms replaced the older "soft-leave + Rejoin" model wholesale — that scaffolding (`softLeaveTab`, `softLeftTabs`, `handleRejoinRoom`, the `rejoin_room` popup envelope, the Rejoin button) is gone. The full arbitration matrix and guard gating live in [`protocol-client.md` §Viewer-driven cursor changes](protocol-client.md#viewer-driven-cursor-changes).

### URL matching is identity-based and adapter-owned

The guard never string-compares URLs. Membership is **by video identity**: `isRoomUrl` resolves the live tab URL to a canonical `videoId` through the *active adapter's* pure, DOM-free `videoIdForUrl(url)` matcher and compares against the cursor + playlist `videoId`s. Each adapter ships that matcher in its own `url` module (e.g. `src/adapters/miruro/url.ts`), registered by adapter id in `src/adapters/url-matchers.ts`; the background imports the registry **without** pulling in the DOM-bound adapter class. A `null` id (wrong site, home, search) counts as off-playlist. This is what absorbs miruro's optional `/watch/<id>/<slug>` slug and lets the share-link `#sync_url=&sync_password=` handoff fragment sit in the address bar without tripping the guard — only identity-bearing parts of the URL are read.
...
The hard-nav target is the cursor's plain canonical `pageUrl` (via `navigableUrlForCursor`, which reconstructs the `?ep=` from the cursor's `videoId` and keeps the show slug) — **no credentials are appended**. They don't need to be: the tab's `pbsync.tab.<tabId>` creds slot persists across the reload and the background's socket survives it (reconnecting from the same slot if it dropped), so the worker keeps the credentials without them ever riding the URL. See [`storage.md`](storage.md).
