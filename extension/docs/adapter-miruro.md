...
- **Cold-start load** — the `<video>` self-loads its blob source on its own (`readyState 4` within ~1 s), but the player also shows a Vidstack `<media-play-button>` **overlay** that hides the video until it is pressed. The button is **independent of the source**: pressing it does not load the source (it self-loads regardless), it only dismisses the overlay so the loaded video becomes playable — and the press only *functions* once a source is present (pressed while still sourceless it is a no-op, so the source is a precondition for a working press). The `strmcx` adapter's `ensurePlayable` override dismisses it — running in the embed frame where that DOM lives: it waits for `media-play-button`, then dispatches a synthesized `Space` keydown/keyup on it (a plain `.click()` is ignored by this control — only keyboard activation dismisses the overlay), then pauses so the room's first command wins. The keyup is delayed ~30 ms: a synchronous release isn't registered, so the player treats Space as *held* and ramps playback to 2× (its hold-to-speed gesture).
