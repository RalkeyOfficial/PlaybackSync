...
- **`scrapeCatalog()`** — for an adapter that can read an episode list (Crunchyroll, etc.). Will arrive as an optional method when the `catalogFragment` reporting spec lands.
