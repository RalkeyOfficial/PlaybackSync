...
- **Randomized display names for anonymous viewers** — connected clients are identified only by opaque `clientId` (UUID) in the dashboard and event log. Generate a Reddit-style handle (random adjective + noun + number, e.g. `WittyFalcon42`) at connect time so the owner can identify viewers at a glance without revealing real identities. Deterministic per-`clientId` so reconnects reuse the same handle.
